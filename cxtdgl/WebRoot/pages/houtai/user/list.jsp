...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		$(document).ready(function(){
		  ...
		  $(".sure").click(function(){
			window.location= "./user-delete.action?entity.id=" + $(".sure_value").val();;
		  $(".tip").fadeOut(100);
		});
	...
	<body>
			...
			<form action="<%=basePath%>user-list.action" method="post" id="listform">
			<div class="rightinfo">
				<div class="tools">
					<ul class="toolbar">
						...
						<s:if test="#request.list!=null">
							<s:iterator value="list">
								<tr>
									...
									<td>
										<c:if test="${isenable == '1'}">否</c:if>
										<c:if test="${isenable == '0'}">是</c:if>
									</td>
									<td>
										<c:if test="${type == '1'}">管理员</c:if>
										<c:if test="${type == '2'}">普通用户</c:if>
									</td>
									<td>
										<s:date name="createtime" format="yyyy-MM-dd" />
									</td>
									<td>
										<a class="tablelink" href="<%=basePath%>user-edit.action?entity.id=<s:property value="id" />">【修改】</a><a class="tablelink" href="javascript:delete_confirm(this,'<s:property value="id" />')">【 删除】</a>
									</td>
								</tr>
							</s:iterator>
						</s:if>
